...
Let:
- a, b, c, d, &isin; &Sigma; (non-terminal)
- A, B, C, D, &isin; V (terminal)
...
parse(alpha, w) = { // returns a sequence of parse trees if alpha =>* w for symbols in alpha
  if (alpha.isEmpty) {
    ...
    (A -> gamma in P).forEach {
      if (parse(alpha, w)) return Some(Seq(tree A where children are parse(gamma, w).get)) // Create new tree node here
    }
    None
  } else { // alpha = A, beta
...
grammars | all | all | most unambiguous grammars | very few practical grammars, no left-associativity
