...
&alpja; **derives** &alpha;<sub>n</sub> (0 or more &rArr; steps) if &alpha;<sub>1</sub> &rArr; ... &rArr; &alpha;<sub>n</sub>
- Use &rArr;\* as a shorthand
- e.g. expr &rArr;\* ID + ID
...
parse(alpha, w) = {
  if (alpha.isEmpty) {
    if (w.isEmpty) true else false
  } else if (alpha == a*beta) { // terminal
    if (w == a*z && parse(beta, z)) true else false
  } else if (alpha == A) {
    (A -> gamma in P).forEach {
      if (parse(alpha, w)) return true
    }
    false
  } else { // alpha = A*beta
    split(w == u*v).forEach {
      if (parse(a, u) && parse(beta, v)) return true
    }
    false
  }
}
